...
abstract class AbstractPost extends \Bakeoff\Wordpress\Model\Entity\PluginEntity
{
    ...
    public function the_content()
    {
        ...
        $content = \Bakeoff\Wordpress\Shortcode\AbstractShortcode::expandContent($content);
        return $content;
    }
     ...
     * @param callable(\Bakeoff\Wordpress\Model\Entity\PluginEntity): string $idGetter receives entity and returns its ID value
     * @param callable(\Bakeoff\Wordpress\Model\Entity\PluginEntity): string $slugGetter receives entity and returns its slug value
    ...
    private function _getPostUrl()
    {
        $permalink_structure = \Cake\Core\Configure::read(
            'Bakeoff/Wordpress.Options.permalink_structure'
        );
        ...
        if (strpos($permalink_structure, '%category%') !== false) {
            ...
            $categories = \Cake\Core\Configure::read('Bakeoff/Wordpress.Categories');
    ...
    private function _getPageUrl()
    {
         ...
         */
        $pages = \Cake\Core\Configure::read($this->getName().'.Pages');
